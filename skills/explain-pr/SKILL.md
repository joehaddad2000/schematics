...
1. Read [overview-format.md](references/overview-format.md) completely and treat it as the normative artifact specification.
2. Resolve the repository, PR number, base SHA, head SHA, and current PR state.
3. Read the title, body, commits, discussion, changed files, and checks.
4. Separate production code, migrations, generated files, tests, documentation, and deployment changes.
5. Inspect the base and head behavior for each important change slice.
6. Trace public inputs through authorization, domain rules, storage, external services, and outputs.
7. Identify the system boundaries, lifecycle, data relationships, API contracts, and rollout order that explain the change.
8. Reuse an existing head-pinned evidence snapshot when one is available for the same PR head.
9. Copy `assets/overview-canvas/` into a temporary or requested output directory and write the PR-specific `overview-data.js` file.
10. Validate every view and interaction against the reference specification.
11. Refresh PR state and checks immediately before handoff.
